...
int main() {
    ...
    bool stop(false);
    monjeu.affiche();
    // Check for piece on the chessboard and draw it
    for (int i = 0; i < NBCOL; i++) {
        for (int j = 0; j < NBCOL; j++) {
            if (monjeu.echiquier.echiquier[i][j] != nullptr) {
                if (monjeu.echiquier.echiquier[i][j]->couleur == 'N') {
                    // Draw the type of the piece
